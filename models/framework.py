...
class GenericModuleMethods(GenericOptions):
    ...
    def _set(self, options):
        key, value = options
        option_item = next((item for item in self.options_list if item['name'] == key), None)
        if len(option_item):
            option_item['value'] = value
            Logger.log('%s --> %s' % (key, value), 'success')
            self.options_info = [[option['display_name'], option['value'], option['required'], option['help']] for option in self.options_list]
    ...
    def _load_opts(self, resource_path=''):
        ...
        if resource_path:
            try:
                ...
                Logger.log('error opening resource file. ensure you use absolute file paths...', 'fail')
