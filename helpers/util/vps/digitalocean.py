...
class DigitalOceanVps():
    ...
    def check_add_key(self, key, fingerprint):
        status, found_key = self.client.get(
            '/v2/account/keys/{}'.format(fingerprint))
        if status == 404:
            Logger.log('pubkey not stored, adding...', 'info')
            addkey_status, addkey_response = self.client.post(
                '/v2/account/keys', params={'name': 'enclave-pubkey', 'public_key': key}, content_type='json')
    ...
    def get_droplets(self):
        ...
        if status == 200:
            ...
            if len(droplets['droplets']):
                ...
                Logger.log('found {} droplets for vps: {}...'.format(
                    len(droplets['droplets']), vps_record.hash[:5]), 'success')
                for droplet in droplets['droplets']:
                    puppet_ip = None
                    if len(droplet['networks']['v4']) > 0:
                        puppet_ip = droplet['networks']['v4'][0]['ip_address']

                    record = Puppet.select().where(
                        Puppet.uuid == droplet['id']).first()
                    if not record:
                        vps_record = VpsInfo.select().where(VpsInfo.api_key == self.api_key).first()
                        ...
                        new_puppet = Puppet(vps='digitalocean', uuid=droplet['id'],
                                            ...
                                            disk=droplet['disk'], ip=puppet_ip, created=Delorean(
                        ).datetime,
                            vps_info_id=vps_record.id, role_id=null_role.id,
                        )
                        new_puppet.save()
                        Logger.log('added droplet {} to db'.format(
                            droplet['id']), 'success')
                    else:
                        ...
                        record.save()
                        Logger.log('updated droplet {}'.format(
                            droplet['name']), 'success')
        else:
            Logger.log('received http code: {} when getting droplets'.format(
                status), 'warning')
    ...
    def get_ip(self, id):
        ...
        if status == 404:
            Logger.log(
                'droplet not available, removing stale db record', 'info')
            ...
            if len(droplet['droplet']['networks']['v4']):
                ...
                Logger.log(
                    'droplet doesn\'t have an ip address yet...', 'info')
        return ip_address

    def delete_droplet(self, uuid):
        delete_status, delete_response = self.client.delete(
            '/v2/droplets/{}'.format(uuid))
        if delete_status == 204:
            Logger.log(
                'droplet {} has been successfully deleted'.format(uuid), 'success')
        else:
            Logger.log('delete task didn\'t return a 204, instead: {}'.format(
    ...
    def create_droplet(self, vps_hash, fingerprint):
        ...
        if create_status == 202:
            ...
            Logger.log('created digitalocean puppet:{}'.format(
                create_response['droplet']['id']), 'success')
